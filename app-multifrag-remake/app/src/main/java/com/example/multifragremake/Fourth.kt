...
class Fourth : Fragment() {
    val args : FourthArgs by navArgs()
    ...
    override fun onCreateView(
        ...
        tv.setText("Waaa, ${args.name} suka ${args.hobby}? Aku juga. Coba kasih tahu makanan favorit mu?")

        btnBackward.setOnClickListener {
            val action = FourthDirections.actionFourthToThird2(args.name, args.asal)
            Navigation.findNavController(view).navigate(action)
        }

        btnForward.setOnClickListener {
            val str = textField.getText().toString()
            val action = FourthDirections.actionFourthToFifth2(
                args.name,
                args.hobby,
                args.asal,
                str
